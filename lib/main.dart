...
/// 교감 상태 메시지 창 queue 항목. [onDismissed] 는 이 메시지가 닫힌 직후 1회 호출된다.
class _InteractionStatusEntry {
  const _InteractionStatusEntry(this.message, {this.onDismissed});
...
/// 영어 교감 문장 삽입용 소문자 동사구 (let’s {feedback}).
String _localizedMealFeedbackActionPhrase(String code) {
  switch (code) {
    case 'protein_up':
    ...
    case 'fat_down':
      return 'reduce fat';
    case 'dietary_fiber_up':
      return 'increase dietary fiber';
    case 'dietary_fiber_down':
      return 'reduce dietary fiber';
    default:
      return '';
  }
}
...
String? _localizedMealFeedbackForInteraction(
  List<String> codes,
  AppLocalizations l10n, {
  required bool isEnglish,
}) {
  if (isEnglish) {
    final phrases = codes
        .map(_localizedMealFeedbackActionPhrase)
        .where((p) => p.isNotEmpty)
        .toList();
    final joined = _joinFeedbackPhrases(phrases, isEnglish: true);
    return joined.isEmpty ? null : joined;
  }
  return _localizedMealFeedbackFromCodes(codes, l10n, isEnglish: false);
}

String _lowercaseFirstAsciiLetter(String text) {
  final trimmed = text.trim();
  if (trimmed.isEmpty) return trimmed;
  final first = trimmed.codeUnitAt(0);
  if (first >= 65 && first <= 90) {
    return '${String.fromCharCode(first + 32)}${trimmed.substring(1)}';
  }
  return trimmed;
}
...
String _formatFeedbackForEnglishSentence(String feedback) {
  ...
  if (!cleaned.contains(',')) return _lowercaseFirstAsciiLetter(cleaned);
  ...
  if (parts.length <= 1) {
    return parts.isEmpty ? cleaned : _lowercaseFirstAsciiLetter(parts.first);
  }
  final normalized = parts.map(_lowercaseFirstAsciiLetter).toList();
  if (normalized.length == 2) return '${normalized[0]} and ${normalized[1]}';
  return '${normalized.sublist(0, normalized.length - 1).join(', ')}, and ${normalized.last}';
}

bool _isHangulSyllableCodeUnit(int codeUnit) {
  return codeUnit >= 0xAC00 && codeUnit <= 0xD7A3;
}

bool _hasKoreanFinalConsonant(String text) {
  final trimmed = text.trim();
  if (trimmed.isEmpty) return false;
  final codeUnit = trimmed.runes.last;
  if (!_isHangulSyllableCodeUnit(codeUnit)) return false;
  return (codeUnit - 0xAC00) % 28 != 0;
}

/// 이름 + 이/가 (비한글 마지막 글자는 `가` fallback).
String _petNameWithSubjectParticle(String name) {
  final trimmed = name.trim();
  if (trimmed.isEmpty) return trimmed;
  final codeUnit = trimmed.runes.last;
  if (!_isHangulSyllableCodeUnit(codeUnit)) return '$trimmed가';
  return _hasKoreanFinalConsonant(trimmed) ? '$trimmed이' : '$trimmed가';
}

/// 이름 + 은/는 (비한글 마지막 글자는 `는` fallback).
String _petNameWithTopicParticle(String name) {
  final trimmed = name.trim();
  if (trimmed.isEmpty) return trimmed;
  final codeUnit = trimmed.runes.last;
  if (!_isHangulSyllableCodeUnit(codeUnit)) return '$trimmed는';
  return _hasKoreanFinalConsonant(trimmed) ? '$trimmed은' : '$trimmed는';
}

String _pickInteractionTemplate(List<String> templates) {
...
String _buildAiStatusMessage({
  ...
  required int affectionGain,
}) {
  final petNameSubject = isEnglish
      ? petName
...
// BottomSheet/Dialog 트리 dispose 등) 와 Dialog 호출 타이밍이 겹치면
...
class _HomePageState extends State<HomePage>
  ...
  /// 교감 상태 메시지 창 (844×390 마당 좌표).
  static const double _kInteractionStatusLeft = 182;
  static const double _kInteractionStatusTop = 264;
  static const double _kInteractionStatusWidth = 480;
  static const double _kInteractionStatusHeight = 80;
  ...
  /// 교감 상태 메시지 창 (식단/놀아주기/쓰다듬기/성장).
  String? _interactionStatusMessage;
  VoidCallback? _interactionStatusCurrentOnDismissed;
  bool _isInteractionStatusOpen = false;
  bool _interactionStatusTransitionInProgress = false;
  bool _interactionStatusDismissRequested = false;
  int _interactionStatusGeneration = 0;
  final List<_InteractionStatusEntry> _interactionStatusQueue =
      <_InteractionStatusEntry>[];
  final Set<String> _shownInteractionStatusEventKeys = <String>{};
  ...
  void initState() {
    ...
    _interactionStatusAnimationController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 220),
      reverseDuration: const Duration(milliseconds: 180),
    );
    _interactionStatusOpacity = CurvedAnimation(
      parent: _interactionStatusAnimationController,
      curve: Curves.easeOutCubic,
      reverseCurve: Curves.easeInCubic,
    );
    _interactionStatusScale = Tween<double>(begin: 0.94, end: 1.0).animate(
      CurvedAnimation(
        parent: _interactionStatusAnimationController,
    ...
    if (targetStage == 'adult' && growthMessage != null) {
      if (_pendingMaturityNoticeBody != null && !deferMaturityNotice) {
        await _showInteractionStatusMessage(
          growthMessage,
          eventKey: growthEventKey,
          onDismissed: _schedulePendingMaturityCompleteNoticeAfterFrame,
        );
      } else {
        await _showInteractionStatusMessage(
          growthMessage,
  ...
  /// 성장 메시지가 닫힌 다음 frame 에 성숭기 완료 모달을 띄운다.
  void _schedulePendingMaturityCompleteNoticeAfterFrame() {
    unawaited(
  ...
  /// [_pendingMaturityNoticeBody] 가 설정되어 있으면 성숭기 완료 모달을 띄운다.
  void _presentPendingMaturityCompleteNotice() {
    if (_pendingMaturityNoticeBody == null) return;
  ...
  Future<void> _handleAdultGraduationIfNeeded({
    ...
    // Modal 은 즉시 띄우지 않고, 성장 메시지가 닫힌 다음에 띄운다.
    final petName = _activePetDisplayName(l10n);
    final petNameSubject = _isEnglishLocale
        ? petName
        : _petNameWithSubjectParticle(petName);
    _pendingMaturityNoticeBody = l10n.maturityCompleteNoticeBody(
      petNameSubject,
    );
  }
  ...
  Future<void> _interactPet(String action) async {
    ...
    if (isPlay && !_hasEligibleMealForToyPlay()) {
      // 놀아주기 자격은 드롭 완료 경로에서 먼저 판정한다. 이중 방어.
      return;
    }
  ...
  void _invalidateAnalysisCache({bool resetPeriod = false}) {
    ...
    if (!mounted) return;
    final shouldRefreshOpenPanel = _isAnalysisPanelOpen && !resetPeriod;
    _safeSetState(() {
      if (resetPeriod) {
    ...
    if (shouldRefreshOpenPanel) {
      unawaited(
        _fetchAnalysisData(period: _selectedAnalysisPeriod, force: true),
      );
    }
  }
  ...
  /// 교감 상태 메시지 창 상태를 즉시 초기화한다.
  void _resetInteractionStatusState({bool resetController = true}) {
    _interactionStatusGeneration++;
    _interactionStatusMessage = null;
    _isInteractionStatusOpen = false;
    _interactionStatusTransitionInProgress = false;
    _interactionStatusDismissRequested = false;
    _interactionStatusQueue.clear();
    _shownInteractionStatusEventKeys.clear();
    if (resetController) {
      _interactionStatusAnimationController.stop();
      _interactionStatusAnimationController.value = 0;
    }
  }

  bool _hasBlockingYardModalOpen() {
    return _isWithdrawConfirmOpen ||
        _isWithdrawFinalConfirmOpen ||
        _isWithdrawErrorNoticeOpen ||
        _isNameInterlockNoticeOpen ||
        _isProfileSelectMissingNoticeOpen ||
  ...
  String? _activePetDisplayNickname() {
    final name = _activePet?['nickname']?.toString().trim();
    if (name == null || name.isEmpty) return null;
    return name;
  }

  String _activePetDisplayName(AppLocalizations l10n) {
    return _activePetDisplayNickname() ?? l10n.interactionPetNameFallback;
  }

  bool _hasEligibleMealForToyPlay() {
    return _todayMealLogs.any((row) {
      final resultType = row['result_type']?.toString().trim().toLowerCase();
      return resultType == 'perfect' || resultType == 'good';
    });
  }

  String _interactionPlaySuccessMessage({required int affectionGain}) {
    final l10n = AppLocalizations.of(context);
    final petName = _activePetDisplayName(l10n);
    final petNameTopic = _isEnglishLocale
        ? petName
        : _petNameWithTopicParticle(petName);
    return _pickInteractionTemplate([
      l10n.interactionPlaySuccess1(petNameTopic, affectionGain),
      l10n.interactionPlaySuccess2(petNameTopic, affectionGain),
    ]);
  }

  String _interactionPlayFailureMessage() {
    final l10n = AppLocalizations.of(context);
    final petName = _activePetDisplayName(l10n);
    final petNameTopic = _isEnglishLocale
        ? petName
        : _petNameWithTopicParticle(petName);
    return _pickInteractionTemplate([
      l10n.interactionPlayFailure1(petNameTopic),
      l10n.interactionPlayFailure2(petNameTopic),
    ]);
  }

  String _interactionPettingSuccessMessage({required int affectionGain}) {
    final l10n = AppLocalizations.of(context);
    final petName = _activePetDisplayName(l10n);
    final petNameSubject = _isEnglishLocale
        ? petName
    ...
    if (family == 'cat') {
      return _pickInteractionTemplate([
        l10n.interactionPettingCat1(petNameSubject, affectionGain),
        l10n.interactionPettingCat2(petName, affectionGain),
      ]);
    }
    // dog 및 미분류는 강아지 문구 사용.
    return _pickInteractionTemplate([
      l10n.interactionPettingDog1(petNameSubject, affectionGain),
      l10n.interactionPettingDog2(petName, affectionGain),
    ]);
  }

  void _enqueueInteractionStatusMessage(
    String message, {
    VoidCallback? onDismissed,
  }) {
    if (onDismissed == null &&
        _interactionStatusQueue.isNotEmpty &&
        _interactionStatusQueue.last.message == message) {
      return;
    }
    _interactionStatusQueue.add(
      _InteractionStatusEntry(message, onDismissed: onDismissed),
    );
  }

  /// 중요한 modal 이 열릴 때 현재 교감 메시지를 queue 앞으로 되돌린다.
  void _stashInteractionStatusForBlockingModal() {
    if (!_isInteractionStatusOpen && !_interactionStatusTransitionInProgress) {
      return;
    }
    final current = _interactionStatusMessage?.trim();
    final currentOnDismissed = _interactionStatusCurrentOnDismissed;
    if (current != null && current.isNotEmpty) {
      if (_interactionStatusQueue.isEmpty ||
          _interactionStatusQueue.first.message != current) {
        _interactionStatusQueue.insert(
          0,
          _InteractionStatusEntry(current, onDismissed: currentOnDismissed),
        );
      }
    }
    _interactionStatusGeneration++;
    _interactionStatusMessage = null;
    _interactionStatusCurrentOnDismissed = null;
    _isInteractionStatusOpen = false;
    _interactionStatusTransitionInProgress = false;
    _interactionStatusDismissRequested = false;
    _interactionStatusAnimationController.stop();
  ...
  Future<void> _presentNextInteractionStatusIfIdle() async {
    if (!mounted) return;
    if (_hasBlockingYardModalOpen()) return;
    if (_isInteractionStatusOpen || _interactionStatusTransitionInProgress) {
      return;
    }
    if (_interactionStatusQueue.isEmpty) return;
    final next = _interactionStatusQueue.removeAt(0);
    await _showInteractionStatusMessage(
      next.message,
      onDismissed: next.onDismissed,
    );
  }

  Future<void> _showInteractionStatusMessage(
    String rawMessage, {
    String? eventKey,
    ...
    if (eventKey != null) {
      if (_shownInteractionStatusEventKeys.contains(eventKey)) return;
      _shownInteractionStatusEventKeys.add(eventKey);
    }

    if (!presentImmediately) {
      _enqueueInteractionStatusMessage(message, onDismissed: onDismissed);
      return;
    }

    if (_hasBlockingYardModalOpen() ||
        _isInteractionStatusOpen ||
        _interactionStatusTransitionInProgress) {
      _enqueueInteractionStatusMessage(message, onDismissed: onDismissed);
      return;
    }

    final generation = ++_interactionStatusGeneration;
    _interactionStatusDismissRequested = false;

    _safeSetState(() {
      _interactionStatusMessage = message;
      _interactionStatusCurrentOnDismissed = onDismissed;
      _isInteractionStatusOpen = true;
      _interactionStatusTransitionInProgress = true;
    });

    _interactionStatusAnimationController.stop();
    _interactionStatusAnimationController.value = 0;
    await _interactionStatusAnimationController.forward();

    if (!mounted) return;
    if (generation != _interactionStatusGeneration) return;

    _safeSetState(() {
      _interactionStatusTransitionInProgress = false;
    });

    if (_interactionStatusDismissRequested) {
      _interactionStatusDismissRequested = false;
      await _hideInteractionStatusMessage();
    }
  }

  Future<void> _hideInteractionStatusMessage() async {
    if (!_isInteractionStatusOpen) return;
    if (_interactionStatusTransitionInProgress) {
      _interactionStatusDismissRequested = true;
      return;
    }

    final generation = ++_interactionStatusGeneration;
    final dismissedCallback = _interactionStatusCurrentOnDismissed;

    _safeSetState(() {
      _interactionStatusTransitionInProgress = true;
    });

    await _interactionStatusAnimationController.reverse();

    if (!mounted) return;
    if (generation != _interactionStatusGeneration) return;

    final nextEntry = _interactionStatusQueue.isEmpty
        ? null
        : _interactionStatusQueue.removeAt(0);
    ...
    dismissedCallback?.call();

    if (nextEntry == null) return;
    if (_hasBlockingYardModalOpen()) {
      _enqueueInteractionStatusMessage(
        nextEntry.message,
        onDismissed: nextEntry.onDismissed,
      );
      return;
    }
  ...
  Widget _buildInteractionStatusMessageOverlay() {
    if (!_isInteractionStatusOpen && !_interactionStatusTransitionInProgress) {
      return const SizedBox.shrink();
    }
    final message = _interactionStatusMessage?.trim() ?? '';
  ...
  Future<void> _showYardNotice({
    ...
    _stashInteractionStatusForBlockingModal();
  ...
  Future<void> _dismissYardConfirmOverlayAnimated(
    ...
    if (_yardConfirmOverlayFadeController.value <= 0) {
      ...
      if (!_hasBlockingYardModalOpen()) {
        unawaited(_presentNextInteractionStatusIfIdle());
      }
      return;
    }
  ...
  Future<void> _completeToyMenuDrop(BagItem toy) async {
    ...
    final userId = supabase.auth.currentUser?.id;
    final petId = _activePet?['id']?.toString() ?? '';
    final attemptId = ++_toyPlayAttemptSeq;
  ...
  Future<void> _applyCompletedMealEvaluationResult(
    ...
    if (resultType == 'perfect' ||
      ...
      await _showInteractionStatusMessage(
        statusMessage,
        eventKey: 'meal:${_todayDateStr()}:$slot:$resultType',
  ...
  /// 식단 인증이 정상 완료됐을 때, 외부 탭 종료와 동일한 모션으로 먹이주기 창을 닫는다.
  Future<void> _dismissMealPanelToYard() async {
    if (_petMealSwapInProgress) return;
    if (!_isMealPanelOpen) return;
  ...
  /// resultType/memo → 식단일지 사진 미리보기 하단 바 문구.
  String? _mealDiaryPhotoResultLabel(String? resultType, String? memo) {
    final l10n = AppLocalizations.of(context);
    final trimmedMemo = memo?.trim();
    switch (resultType?.trim().toLowerCase()) {
      case 'perfect':
        return l10n.mealDiaryPhotoResultPerfect;
      case 'good':
        return (trimmedMemo != null && trimmedMemo.isNotEmpty)
            ? l10n.mealDiaryPhotoResultGood(trimmedMemo)
            : l10n.mealDiaryPhotoResultGoodPlain;
      case 'bad':
        return (trimmedMemo != null && trimmedMemo.isNotEmpty)
            ? l10n.mealDiaryPhotoResultBad(trimmedMemo)
            : l10n.mealDiaryPhotoResultBadPlain;
      default:
        return null;
    }
  }
  ...
  // - 바깥 터치 또는 우상단 X 로 닫기
  // - resultType/memo 가 있으면 사진 하단에 당시 식단 결과·피드백 바를 표시한다.
  Future<void> _showMealPhotoPreview(
    String imageUrl, {
    String? resultType,
    String? memo,
  }) async {
    if (!mounted) return;
    final resultLabel = _mealDiaryPhotoResultLabel(resultType, memo);
                      ...
                      Row(
                        ...
                        children: [
                          _photoSlot(
                            ...
                            url: _brunchUrl,
                            resultType: _logForSlot(
                              'brunch',
                            )?['result_type']?.toString(),
                            memo: _logForSlot('brunch')?['memo']?.toString(),
                          ),
                          _photoSlot(
                            ...
                            memo: _logForSlot('dinner')?['memo']?.toString(),
                          ),
                        ],
                      ),
                      const SizedBox(height: 8),
  ...
  Widget _photoSlot({
    ...
    return SizedBox(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(20),
        onTap: hasPhoto
            ? () => widget.onPhotoTap(url, resultType: resultType, memo: memo)
            : null,
        child: Container(
          decoration: BoxDecoration(
